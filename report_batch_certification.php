		...
		while($centre = tep_db_fetch_array($center_query)){
			...
			while($courses = tep_db_fetch_array($courses_query)){
				...
				while($batches = tep_db_fetch_array($batches_query)){
					$students_query_raw = "select student_full_name, student_father_name, student_surname, student_gender, student_category, is_minority_category, course_option, student_mobile, student_district, if(is_training_completed = '1', 'Y', 'N') as is_training_completed, training_dropout_reason, if(is_apeared_for_test = '1', 'Y', 'N') as is_apeared_for_test, test_result, if(is_certificate_recieved = '1', 'Y', 'N') as is_certificate_recieved, is_apeared_for_test, test_result, test_absent_reason, batch_id, test_allotted_date, test_abn_no, test_agency, is_form_uploaded_sdi_web from " . TABLE_STUDENTS . " where centre_id = '" . $centre['centre_id'] . "' and course_id = '" . $courses['course_id'] . "' and batch_id = '" . $batches['batch_id'] . "'";
					...
					while($students = tep_db_fetch_array($students_query)){
						...
						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, ($students['is_apeared_for_test'] == '1' ? 'Y' : 'N'));
						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (isset($students['test_result']) && $students['test_result'] != '' ? $arr_exam_result[$students['test_result']] : ''));
						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $students['is_certificate_recieved']);

						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, display_valid_date($students['test_allotted_date']));
						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $students['test_abn_no']);
						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $students['test_agency']);
